...
**示例：**
```
auto 搜索 Gemma4 的详细信息，给出一份本地部署报告
```

---

### 方式 2: 使用 tmux-auto-agent.sh / tmux-sub-agent.sh 脚本

**⚠️ 重要：参数格式必须正确！**

```bash
# Auto Agent - 正确格式
bash /home/wuyangcheng/.qwen/scripts/tmux-auto-agent.sh <session-name> "<任务描述>"

# Sub Agent - 正确格式
bash /home/wuyangcheng/.qwen/scripts/tmux-sub-agent.sh <session-name> "<任务描述>"
```

**参数说明：**
| 参数 | 说明 | 示例 |
|------|------|------|
| `<session-name>` | **第 1 个参数** - tmux 会话名称（英文简写） | `qwen-auto-gemma4` |
| `<任务描述>` | **第 2 个参数** - 任务指令（用引号包裹） | `"搜索 Gemma4 详情并生成报告"` |

**⚠️ 常见错误：**
```bash
# ❌ 错误：缺少 session name 参数，会导致任务无法正确启动
bash /home/wuyangcheng/.qwen/scripts/tmux-auto-agent.sh "任务描述"
```

**Session Name 命名规范：**
- 必须以 `qwen-auto-` 或 `qwen-sub-` 开头
- 使用**英文**简写，方便后续监控和发送命令
- 示例：`qwen-sub-analysis`, `qwen-auto-scorer100`, `qwen-auto-gemma4`
...
| 脚本 | 路径 | 功能 |
|------|------|------|
| **tmux-sub-agent.sh** | `/home/wuyangcheng/.qwen/scripts/tmux-sub-agent.sh` | 🔄 启动 Sub Agent |
| **tmux-auto-agent.sh** | `/home/wuyangcheng/.qwen/scripts/tmux-auto-agent.sh` | 🔄 启动 Auto Agent |
...
所有具体project相关任务的代码和总结报告都必须保存到 `/home/wuyangcheng/code/<project_name>/` 目录中。

**详细文档**: `/home/wuyangcheng/.qwen/<project_name>_PROJECT.md`

### 目录结构

```
/home/wuyangcheng/code/<project_name>/
├── src/                    # 源代码文件
├── docs/                   # 文档和报告
├── results/                # 结果文件
├── configs/                # 配置文件
...
- [prepare-sub-agent.sh](../scripts/prepare-sub-agent.sh) - 📋 准备并启动子进程
