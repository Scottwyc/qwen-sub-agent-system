...
# 检测是否为 scorer 相关任务，如果是则使用 scorer-searcher 目录
if [[ "$TASK" == *"scorer"* ]] || [[ "$TASK" == *"Scorer"* ]] || [[ "$SESSION_NAME" == *"scorer"* ]]; then
    cat >> "$WORKPLAN_FILE" << 'EOF'
#### Scorer-Searcher 项目专用目录

**所有文件必须保存到 `/home/wuyangcheng/code/scorer-searcher/` 目录！**

详细文档：`/home/wuyangcheng/.qwen/SCORER_SEARCHER_PROJECT.md`

| 文件类型 | 保存位置 | 示例 |
|----------|----------|------|
| 代码文件 | `/home/wuyangcheng/code/scorer-searcher/src/` | `src/data_generator.py` |
| 报告文档 | `/home/wuyangcheng/code/scorer-searcher/docs/` | `docs/n50k4p01_report.md` |
| 结果文件 | `/home/wuyangcheng/code/scorer-searcher/results/{task_name}/` | `results/n50k4p01/model.pt` |
| 配置文件 | `/home/wuyangcheng/code/scorer-searcher/configs/` | `configs/train_config.json` |
| 脚本文件 | `/home/wuyangcheng/code/scorer-searcher/scripts/` | `scripts/train.sh` |
| 任务总结 | `/home/wuyangcheng/code/scorer-searcher/` (根目录) | `TASK_SUMMARY_n50k4p01_20260401.md` |

**工作流程文档**: `/home/wuyangcheng/code/scorer-searcher/docs/WORKFLOW_DATA_GENERATION_TRAINING.md`
...
| 文件类型 | 保存位置 | 示例 |
|----------|----------|------|
| 代码文件 | `/home/wuyangcheng/code/` | `data_generator.py` |
| 数据集 | `/home/wuyangcheng/code/dataset*/` | `dataset/multi_seed.pt` |
| 模型结果 | `/home/wuyangcheng/code/result/` | `result/scorer_training/` |
| 报告文档 | `/home/wuyangcheng/code/` | `<task>_report.md` |
| 可视化 | `/home/wuyangcheng/code/result/` | `result/figure.png` |
...
# 添加 Scorer-Searcher 项目约定（如果任务涉及 scorer）
if [[ "$TASK" == *"scorer"* ]] || [[ "$TASK" == *"Scorer"* ]] || [[ "$SESSION_NAME" == *"scorer"* ]]; then
    SYSTEM_PROMPT="${SYSTEM_PROMPT}

📁 Scorer-Searcher 项目约定（重要！）：
...
保存规则：
- 代码文件：/home/wuyangcheng/code/scorer-searcher/src/{task_name}.py
- 报告文档：/home/wuyangcheng/code/scorer-searcher/docs/{task_name}_report.md
- 结果文件：/home/wuyangcheng/code/scorer-searcher/results/{task_name}/
- 任务总结：/home/wuyangcheng/code/scorer-searcher/TASK_SUMMARY_{task_name}_{date}.md

工作流程：
1. 数据生成优先使用 data_generator_direct 等方式获得更广泛的 F1 分布
2. 进行 F1 分布可视化分析
3. 确认 F1 分布合理后再进行 scorer 训练

详细流程：/home/wuyangcheng/code/scorer-searcher/docs/WORKFLOW_DATA_GENERATION_TRAINING.md"
fi
...
cat > "$SYSTEM_PROMPT_FILE" << 'SYSTEM_PROMPT_START'
...
if [ -n "$WORKPLAN_CONTENT" ]; then
    ...
    echo "📋 最新工作计划：" >> "$SYSTEM_PROMPT_FILE"
    cat "$WORKPLAN_FILE" >> "$SYSTEM_PROMPT_FILE"
fi

# 添加 compress 总结到 prompt 文件
if [ -n "$COMPRESS_SUMMARY" ]; then
    echo "" >> "$SYSTEM_PROMPT_FILE"
    echo "📝 主进程会话总结（compress summary）：" >> "$SYSTEM_PROMPT_FILE"
    echo "$COMPRESS_SUMMARY" >> "$SYSTEM_PROMPT_FILE"
fi

# 添加 Scorer-Searcher 项目约定（如果任务涉及 scorer）
if [[ "$TASK" == *"scorer"* ]] || [[ "$TASK" == *"Scorer"* ]] || [[ "$SESSION_NAME" == *"scorer"* ]]; then
    cat >> "$SYSTEM_PROMPT_FILE" << 'SCORER_PROJECT_INFO'
